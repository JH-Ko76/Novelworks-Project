...
想定として、1日ユーザー数を約1,000人、ユーザー1人あたり平均20回のリクエストが発生すると仮定した場合、
月額およそ1万円程度でサービスを運用できると考えています。


その理由として、AWS Lambda や DynamoDB は無料利用枠が比較的大きく、上記の利用規模であれば大きな課金は発生しにくいと想定しています。
また、ログ管理やAPIキー管理についても、コストへの影響は限定的であると考えています。


本システムのコストの約70〜80%はAIトークン使用量が占めると想定しています。


実際の開発過程においても、無料トークンの上限が想定より少なく、短時間で利用回数を消費してしまい、翌日に持ち越して検証を行うケースが多くありました。


今回のプロジェクトを通じてAI APIを活用したプロジェクトにおいては、プロンプトの最適化やキャッシュの導入によるトークン使用量の削減が、コスト最適化における重要なポイントであると学びました。
