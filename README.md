...
想定として、1日あたりのユーザー数を約1,000人、ユーザー1人あたり平均20回のリクエストが発生すると仮定した場合、
...
したがって、本システムにおける主要なコスト要因は、AIのトークン使用量であると認識しています。


実際の開発過程においても、無料トークンの上限が想定より少なく、短時間で利用回数を消費してしまい、翌日に持ち越して検証を行うケースが多くありました。


この経験から、本番運用を見据える場合には、トークン使用量を最小化するためのロジック改善が必要であると考えています。
