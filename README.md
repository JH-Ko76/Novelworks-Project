...
また、問い合わせの分類結果が曖昧で確認が必要な場合には、画像のように修正ボタンを使用して内容を編集し、そのまま保存できる機能を実装しました。
...
- API Gateway
     ...
     (1).セキュリティ & 可用性 </br>
     本システムでは、Lambda関数がパブリックインターネットに直接公開されないよう、
     API Gatewayをエントリーポイントとして採用しました。
     これにより、外部からのアクセスを一元的に制御できる構成としています。
  
     
     (2).拡張性 </br>
     今後サービスが拡張され、複数のLambda関数や機能が追加された場合でも、
     API Gatewayを単一のエンドポイントとして利用することで、クライアント側の変更を最小限に抑えられると考えました。
...
- API Gatewayへの認証機構の導入 </br>
  一方で、現時点の実装では認証機構は未導入であり、APIは誰でもアクセス可能な状態となっています。 
  そのため、本番運用を見据えた場合には、認証・認可機構の導入が必須であると認識しています
...
- AI APIの利用制限について </br>
    このシステムの主な課金ポイントは、AIのトークン使用量によって決まると考えています。
    実際にサービスを開発する過程で、無料トークンの量が想像以上に少なく、
    すぐに会話回数を使い切ってしまい、翌日に改めてテストすることが多くありました。
    そのため、実際にサービスを運用する際には、トークン使用量を最小限に抑えるロジックへ
    改善する必要があると感じました。
...
AWS Secrets Manager（APIキー管理）</br>
Amazon CloudWatch（ログ管理）</br>
API Gateway（APIリクエスト処理）



想定として、1日あたりのユーザー数を約1,000人、1ユーザーあたり平均20回のリクエストが発生すると仮定した場合、月間の総リクエスト数は約60万回となります。 


この場合、Gemini Flashの料金を基準にすると、100万トークンあたり約0.30ドルであり、1リクエストあたり約500トークン（入力・出力合計）と仮定すると、月間のトークン使用量は約3億トークンとなります。 
その結果、AIサービスにかかる費用はおよそ1,600円〜1,800円程度になると想定されます。 


一方で、AWS LambdaやDynamoDBについては無料利用枠が比較的大きく、上記の利用規模であれば大きな課金は発生しにくいと考えています。 
また、CloudWatchによるログ管理やSecrets ManagerによるAPIキー管理についても、コストへの影響は限定的であると想定しています。 

これらを踏まえると、本システムの月額コストは全体で数千円〜1万円前後に収まる可能性があると考えています。 
特に、全体コストの約70〜80%はAIトークン使用量が占めると見込んでいます。 


実際の開発過程においても、無料トークンの上限が想定より少なく、短時間で利用回数を消費してしまい、翌日に持ち越して検証を行うケースが多くありました。 
この経験から、AI APIを活用したシステムにおいては、プロンプトの最適化やキャッシュの導入によるトークン使用量の削減が、コスト最適化において重要なポイントであると学びました。
